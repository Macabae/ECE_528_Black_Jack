...
void EUSCI_B1_I2C_Init()
{
    ...
    // the USCYNC bit (bit 8) in the UCBxCTLW0 register
    ...
    // Clear the UCTXNACK bit (Bit 3) in the UCBxCTLW0 register
    EUSCI_B1->CTLW0 &= ~0x0008;

    // Configure the EUSCI_B1 module not to generate the STOP condition in master mode by cleraing
    ...
    // the UCTXSTT bit (Bit 1) in the UCBxCTLW0 register. Note that in master receive mode,
...
void EUSCI_B1_I2C_Send_A_Byte(uint8_t slave_address, uint8_t data)
{
    ...
    // Asign the slave device's address to the UCBxI2CSA register
...
{
    // Wait until the EUSCI_B1 module is not busy by checking hte
    // UCBBUSY bit (Bit 4) in the UCBxSTATw register
    while((EUSCI_B1->STATW & 0x0010) != 0);
    ...
    // Set the UTCR bit (Bit 4) in the UCBxCTLW0 register to configure the EUSCI_B1 module \
    // in master transmitter mode. Then, clear the UCTXSTP bit (Bit 2) to not generate the STOP condition
    // Lastly, set the UCTXSTT bit (Bit 1) to generate the START condition
    EUSCI_B1->CTLW0 = (EUSCI_B1->CTLW0 & ~0x0004) | 0x0012;
    ...
    for (i = 0; i < packet_length; i++)
    {
        ...
        // Store the 8-bit data in the trnasmit Buffer by writing the data
        // to the UCBxTXBUF register
    ...
    // Ensure that the transmit interrupt flag is not set by cleraing the
    // UCTXIFG0 bit (Bit 1) in the UCBxIFG register
    EUSCI_B1->IFG &= ~0x0002;
}
...
uint8_t EUSCI_B1_I2C_Receive_A_Byte(uint8_t slave_address)
{
    ...
    // Set the byte counter threshold to 1 by writing the UCTBCNTx field in the UCBxTBCNT register.
    // This register can only be modified when the UCSWRST bit (Bit 0) in the UCBxCTLW0 reigster is set
    ...
    // Return the received data from the Receive Buffer and ensure nthat it has a type of uint8_t
...
{
    // Assign the slave device's address to the UCBxI2SCA register
    EUSCI_B1->I2CSA = slave_address;

    // Clear the UCTR bit (Bit 4) in the UCBxCTLW0 register to configure the EuscIC_B1 module
    ...
    // Use a loop to transfer the data individually fromt he Receive Buffer to the array
    int i;
    for (i = 0; i < packet_length; i++)
    {
...
void OV7670_Write_Reg(uint8_t reg, uint8_t data) {
    ...
    buf[0] = reg;
    buf[1] = data;
    // 0x21 is the OV7670 Slave Address
    EUSCI_B1_I2C_Send_Multiple_Bytes(0x21, buf, 2);
    Clock_Delay1ms(1); // Small safety delay
}

void OV7670_Init_Sequence(void) {
    // 1. Reset the camera
    OV7670_Write_Reg(0x12, 0x80);
    Clock_Delay1ms(30);

    // 2. Clock & Basic Settings
    OV7670_Write_Reg(0x11, 0x01); // Prescaler (adjusts PCLK speed)
    OV7670_Write_Reg(0x12, 0x04); // COM7: Output RGB
    OV7670_Write_Reg(0x40, 0xD0); // COM15: RGB565, Full Range

    // 3. Scaling to QQVGA (160x120)
    // This allows the MSP432 to capture a full frame into RAM
    OV7670_Write_Reg(0x0C, 0x04); // COM3: Enable Scaling
    OV7670_Write_Reg(0x3E, 0x19); // COM14: Manual scaling, PCLK divided by 4
    OV7670_Write_Reg(0x70, 0x3A); // SCALING_XSC
    OV7670_Write_Reg(0x71, 0x35); // SCALING_YSC
    OV7670_Write_Reg(0x72, 0x11); // SCALING_DCWCTR
    OV7670_Write_Reg(0x73, 0xF1); // SCALING_PCLK_DIV
    OV7670_Write_Reg(0xA2, 0x02); // SCALING_PCLK_DELAY

    // 4. Color & Light Balance (Standard defaults)
    OV7670_Write_Reg(0x13, 0x07); // COM8: Enable AEC, AGC, AWB
    OV7670_Write_Reg(0x01, 0x40); // Blue Gain
    OV7670_Write_Reg(0x02, 0x60); // Red Gain
}
